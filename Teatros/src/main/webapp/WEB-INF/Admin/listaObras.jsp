...
<%@ page import="java.util.*, com.teatro.modelo.*, java.text.SimpleDateFormat" %>
<%
...
<head>
    ...
    <style>
        ...
        .funciones-container { background: #f8f9fa; border-radius: 10px; padding: 10px; margin-top: 10px; flex-grow: 1; }
        .funcion-row { 
        ...
        .funcion-row:hover { background: #e8f4fd; border-color: #3498db; }
        
        .btn-delete-fn { color: #e74c3c; padding: 5px 8px; border-radius: 5px; transition: 0.2s; }
        .btn-delete-fn:hover { background: #fdeaea; color: #c0392b; }
        ...
        .btn-nueva-funcion { font-size: 0.75rem; text-decoration: none; font-weight: bold; color: #1abc9c; transition: 0.3s; }
        .btn-nueva-funcion:hover { color: #16a085; }
    </style>
</head>
<body>
...
<div class="container py-4">
    <div class="header-admin shadow">
        ...
        <div class="d-flex gap-2">
            <a href="nuevaObra" class="btn btn-success btn-sm"><i class="fas fa-plus-circle"></i> Nueva Obra</a>
            <a href="adminDashboard" class="btn btn-outline-light btn-sm"><i class="fas fa-arrow-left"></i></a>
        </div>
    </div>
    ...
    <%
        ...
        if (status != null) {
            if (status.equals("success_delete_funcion")) { %>
                <div class="alert alert-success alert-dismissible fade show shadow-sm">
                    <i class="fas fa-check-circle me-2"></i> Función eliminada correctamente.
                    <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
                </div>
            <% } else if (status.equals("error_entradas")) { %>
                <div class="alert alert-warning alert-dismissible fade show shadow-sm">
                    <i class="fas fa-exclamation-triangle me-2"></i> <strong>No se puede eliminar:</strong> Esta función tiene entradas vendidas.
                    <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
                </div>
            <% }
        }
                ...
                <div class="card-obra">
                    ...
                    <div class="card-body">
                        ...
                        <div class="d-flex justify-content-between align-items-center mb-1">
                            <label class="small fw-bold text-secondary text-uppercase">Funciones:</label>
                            <a href="nuevaFuncion?obraId=<%= obra.getId() %>" class="btn-nueva-funcion">
                                <i class="fas fa-calendar-plus"></i> AGREGAR FUNCION
                            </a>
                        </div>
                        ...
                        <div class="funciones-container">
                            <% 
                                ...
                                if (funciones != null && !funciones.isEmpty()) {
                                    Collections.sort(funciones, new Comparator<Funcion>() {
                                        @Override
                            ...
                            %>
                                <div class="funcion-row shadow-sm">
                                    <a href="verEstadoFuncion?id=<%= f.getId() %>" class="text-decoration-none text-dark" style="flex-grow: 1;">
                                        <div class="small">
                                    ...
                                    <div class="d-flex align-items-center">
                                        <a href="<%= request.getContextPath() %>/eliminarFuncion?id=<%= f.getId() %>" 
                                           class="btn-delete-fn"
                                           onclick="return confirm('¿Seguro que deseas eliminar la función del <%= sdfFecha.format(f.getFecha()) %>?')">
                            ...
                            <%      }
                                ...
                                <p class="text-center text-muted small py-2 mb-0">Sin funciones próximamente.</p>
                            <% } %>
                        </div>
                    </div>
                    
                    <div class="card-footer-actions">
                        <a href="editarObra?id=<%= obra.getId() %>" class="action-link text-warning"><i class="fas fa-edit"></i> EDITAR</a>
                        <a href="eliminarObra?id=<%= obra.getId() %>" class="action-link text-danger" onclick="return confirm('¿Eliminar esta obra?')">
